        ...
        async.series(fns, function(err) {
            ...
            async.series(fns, function(err) {
                ...
                for (var i = 0; i < beforeHandlers.length; i++) {
                    ...
                    var fn = function(deploymentPackage, beforeHandler, manifest, multistore) {
                        return function(done) {
                            beforeHandler(deploymentPackage, manifest, multistore, function(err) {
                                done(err);
                            });
                        };
                    }(deploymentPackage, beforeHandler, manifest, multistore);
                    fns.push(fn);
                }
